...
def load_data() -> pd.DataFrame:
    kwargs = dict(thousands=",", encoding="latin-1")
    if SOURCE == "url":
...
# === retorna uma lista de colunas numéricas, excluíndo a coluna alvo
def get_feature_columns(dataframe: pd.DataFrame) -> list:
    colunas_numericas = dataframe.select_dtypes(include=[np.number]).columns.drop(IGNORE_COLS)
...
def train_tune(pipelines: dict, params: dict, x_train: pd.DataFrame, y_train: pd.Series) -> dict:
    ...
    for name, pipeline in pipelines.items():
        ...
        grid = GridSearchCV(
            estimator=pipeline,
            param_grid=params[name], #params.get(name, {}),
            cv=5,
        ...
        print(f"   -> melhor cv: {grid.best_score_:.4f} ({grid.best_score_ * 100:.2f}%)")
...
def select_best_model(results: dict, models: dict):
    ...
    print(f"Melhor modelo: {best_name} com acurácia de {best_score:.4f} ({best_score * 100:.2f}%)")
...
def plot_graphs(platform_correlation, dataframe, artist_metrics):
    print("\n" + "-" * 70 + "\n")
...
def main():
    ...
    # 8 - plota os graficos do EDA (Opcional)
    if eval(PLOT_GRAPH):
        plot_graphs(platform_correlation, dataframe, artist_metrics)
